...
def Gate_Publisher(pair_no,st):
    ...
    if pair_no==1 :    #odd number pair 
        ...
        if z == 1:
            ...
            x_target = Mod_contact(1, legfix_final_cg ,legvar_final_cg)  
            print("x target is "+ str(x_target))  
            trajectory_modification(x_current, y_current, x_target, cycletime_required,indx_fix,pair_no)
            z=0
        ...
        if z == 1:
            ...
            legfix_final_cg[1] = legfix_initial_cg[1]                # Must be relative to cg
            legvar_final_cg[1] = legvar_initial_cg[1]
            x_target = Mod_contact(2, legfix_final_cg ,legvar_final_cg)    
            print("x target is "+ str(x_target))
            trajectory_modification(x_current, y_current, x_target, cycletime_required,indx_fix,pair_no)
            z = 0                    
