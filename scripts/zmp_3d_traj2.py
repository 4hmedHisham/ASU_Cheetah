...
def Gate_Publisher_3D(pair_no,xpoint_f,ypoint_f,xpoint,ypoint):
    ...
    if pair_no==1 :    #odd number pair     
        ...
        while(1):
            ...
            if ((current - last_fix) > sample_time_f ) and i < steps:        
                last_fix = current
                trans1,hip1,knee1 = gait.inverse_kinematics_3d_v6(x_plane_f[i], y_plane_f[i], z_plane_f[i],legfix_Prev_angs[0] ,legfix_Prev_angs[1], legfix_Prev_angs[2],L_f)
                trans3,hip3,knee3 = gait.inverse_kinematics_3d_v6(x_plane[i], y_plane[i], z_plane[i],legvar_Prev_angs[0] ,legvar_Prev_angs[1], legvar_Prev_angs[2],L_v )
                if flaag == 1:
                ...
                set_angle(8, knee1) 
                # Move Body                                    
                set_angle(3,trans4[i])
                set_angle(4, hip4[i])
                set_angle(5, knee4[i])         
                set_angle(9,trans2[i])
                set_angle(10 , hip2[i])
                set_angle(11, knee2[i])
                if(z == 1):
                    x_current = x_plane[i]
                    y_current = y_plane[i]
                    z_current = z_plane
        ...
        while(1):
            ...
            if ((current - last_fix) > sample_time_f ) and i < steps:        
                ...
                set_angle(5, knee4) 
                # Move Body                                    
                set_angle(0,trans3[i])
                set_angle(1, hip3[i])
                set_angle(2, knee3[i])         
                set_angle(6,trans1[i])
                set_angle(7, hip1[i])
                set_angle(8, knee1[i])
                if(z == 1):
                    x_current = x_plane[i]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():        
        ...
        #     Gate_Publisher_3D(1,300,0,300,0)
        # if first_step_flag == 1:
        #     x,y,z =three_d_trajv3(1,150,0)
        #     move_leg(1,x,y,z)
        #     time.sleep(0.2)
        #     print("Final 1 position is: "+str(leg_pos1_hip[0]))
        #     x,y,z =three_d_trajv3(2,150,0)
        #     move_leg(2,x,y,z)
        #     time.sleep(0.2)
        #     print("Final 2 position is: "+str(leg_pos2_hip[0]))
        #     x,y,z =three_d_trajv3(3,150,0)
        #     move_leg(3,x,y,z)
        #     time.sleep(0.2)
        #     print("Final 3 position is: "+str(leg_pos3_hip[0]))            
        #     x,y,z =three_d_trajv3(4,150,0)
        #     move_leg(4,x,y,z)
        #     time.sleep(0.2)            
        #     print("Final 4 position is: "+str(leg_pos4_hip[0]))            
        #     first_step_flag = 0      
